...
function loadCorpus() {
  for (const conf of listConferences()) {
    ...
    CORPUS[conf.id] = { list, byId: new Map(list.map((p) => [p.id, p])) };
  }
}
loadCorpus();

const corpusOf = (conf) => CORPUS[conf] || { list: [], byId: new Map() };
...
export async function stats(conference) {
  const total = corpusOf(conference).list.length;
  const { data, error } = await supabase.from('decisions').select('decision').eq('conference', conference);
  if (error) throw new Error(error.message);
  const byKind = (data || []).reduce((acc, r) => ((acc[r.decision] = (acc[r.decision] || 0) + 1), acc), {});
...
export async function nextUndecided(conference) {
  ...
  for (const p of list) if (!decided.has(p.id)) return p;
  return null;
}
...
export async function listPapers(conference, { status = 'like', q = '' } = {}) {
  ...
  if (status === 'undecided') {
    return list.filter((p) => !decided.has(p.id) && matches(p)).map((p) => withDecision(p, null));
  }
